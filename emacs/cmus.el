...
"Start playing the current song in cmus."
  (interactive)
  (start-process-shell-command "*CMUS-Shell-Play*" "*CMUS-Shell-Output*" (ssh-command-string) "-p"))
(global-set-key "\C-c\C-xp" 'cmus-play)

(defun cmus-pause ()
"Pause playing the current song in cmus."
  (interactive)
  (start-process-shell-command "*CMUS-Shell-Pause*" "*CMUS-Shell-Output*" (ssh-command-string) "-u"))
(global-set-key "\C-c\C-xu" 'cmus-pause)

(defun cmus-stop ()
"Stop playing the current song in cmus."
  (interactive)
  (start-process-shell-command "*CMUS-Shell-Stop*" "*CMUS-Shell-Output*" (ssh-command-string) "-s"))
(global-set-key "\C-c\C-xs" 'cmus-stop)
...
NUM is a number in (0..100)."
  ...
  (start-process-shell-command "*CMUS-Shell-Vol*" "*CMUS-Shell-Output*" (ssh-command-string) "-v" volume))
(global-set-key "\C-c\C-xv" 'cmus-vol)

(defun cmus-next ()
"Play the next song in cmus."
  (interactive)
  (start-process-shell-command "*CMUS-Shell-Next*" "*CMUS-Shell-Output*" (ssh-command-string) "-n"))
(global-set-key "\C-c\C-xn" 'cmus-next)

(defun cmus-previous ()
"Play the previous song in cmus."
  (interactive)
  (start-process-shell-command "*CMUS-Shell-Previous*" "*CMUS-Shell-Output*" (ssh-command-string) "-r"))
(global-set-key "\C-c\C-xr" 'cmus-previous)

(defun cmus-show-lyrics ()
"Retrieve the lyrics of the song currently played and display it in the buffer *CMUS Lyrics*."
   (interactive)
   (switch-to-buffer (create-file-buffer "*CMUS Lyrics*"))
   (text-mode)
   (insert (shell-command-to-string (ssh-command-lyrics-string)))
   (goto-char (point-min))
   (view-mode))
(global-set-key "\C-c\C-xl" 'cmus-show-lyrics)

(defun cmus-show-lyrics-user (&optional artist title)
"Retrieve the lyrics of the ARTIST and TITLE provided by the user and displays it in the buffer *CMUS Lyrics*."
  (interactive
   "Martist: \nMtitle: ")
  (switch-to-buffer (create-file-buffer "*CMUS Lyrics*"))
  (text-mode)
  (insert
   (shell-command-to-string
    (concat (ssh-command-lyrics-string)
	    (shell-quote-argument
	     (concat " \""artist"\"" " " "\""title"\"")))))
  (goto-char (point-min))
  (view-mode))
(global-set-key "\C-c\C-xt" 'cmus-show-lyrics-user)
...
"Show the current state of cmus."
  ...
  (goto-char (point-min))
  (view-mode))
(global-set-key "\C-c\C-xQ" 'cmus-status)
;------------------------------------------------------------------------------
